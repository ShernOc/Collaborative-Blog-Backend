...
def update_user():
    ...
    if not user:
        return jsonify({"Error":"Login to Update"}), 404
    ...
    if check_name and check_email:
        return jsonify({"Error": "Name or Email already exist"}),406
    ...
    if name==user.name and email==user.email:
        return jsonify({"Error":"No change detected in your update"}), 400
            ...
            #commit the function 
            db.session.commit()
            return jsonify({"Success": f"User was updated successfully"}),200
...
def delete_all_user(user_id):
    ...
    if user_delete.id == current_user.id:
        ...
        db.session.commit()
        return jsonify({"Success":f"User deleted successfully"}), 200
