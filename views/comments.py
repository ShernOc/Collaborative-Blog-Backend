...
def post_comments():
    ...
    check_content = Comment.query.filter_by(content = content, user_id = user_id).first()
    
    if check_content: 
        return jsonify({"Error":"The comments already exist"}), 400
    else: 
        new_comment = Comment(blog_id = blog_id, user_id = user_id, content=content)
...
#Delete a Comment  
@comment_bp.route('/comments/<int:user_id>' ,methods=['DELETE'])      
